...
func (s *wordHandler_Suite) Test_wordHandler_deleteWord() {
	...
	tests := []struct {
		...
		},
		{
			...
			setupMock: func() {
				s.srv.On("DeleteWordFromCollection", mock.Anything, mock.Anything).Once().Return(
		...
		},
		{
			...
			setupMock: func() {
				s.srv.On("DeleteWordFromCollection", mock.Anything, mock.Anything).Once().Return(nil)
			},
		},
	}
	...
	for _, tc := range tests {
		s.Run(tc.Name, func() {
			tc.setupMock()
			s.h.deleteWordFromCollection(tc.Args.w, tc.Args.r)
