...
func (p *Postgres) IsWordInCollection(ctx context.Context,
	...
	if err != nil {
		return false, fmt.Errorf("Postgres - IsWordInCollection - ToSql: %w", err)
	}

	var inColl bool
	err = p.Pool.BeginFunc(ctx, func(tx pgx.Tx) error {
		if err := tx.QueryRow(ctx, sql, args...).Scan(&inColl); err != nil {
			return fmt.Errorf("Postgres - IsWordInCollection - Scan: %w", err)
		}
		return nil
	})
	if err != nil {
		return inColl, fmt.Errorf("Postgres - IsWordInCollection - BeginFunc: %w", err)
	}
