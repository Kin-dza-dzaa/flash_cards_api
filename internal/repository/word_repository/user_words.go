...
func (p *WordRepository) UserWords(ctx context.Context,
	...
	err = p.Pool.BeginFunc(ctx, func(tx pgx.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("WordRepository - UserWords - Query: %w", err)
		}
		defer rows.Close()
		...
		for rows.Next() {
			if err := rows.Scan(&collectionName, &wordData.TimeDiff, &wordData.LastRepeat, &wordData.WordTrans); err != nil {
				return fmt.Errorf("WordRepository - UserWords - Scan: %w", err)
			}
	...
	if err != nil {
		return nil, fmt.Errorf("WordRepository - UserWords - BeginFunc: %w", err)
	}
